...
##################################################################################
# Setup VPC, subnets, and internet access to the VPC
##################################################################################
...
resource "aws_instance" "elastic_nodes" {
  ...
  key_name               = aws_key_pair.elastic_ssh_key.key_name #"elk_key"
...
resource "null_resource" "move_elasticsearch_file" {
  ...
  connection { 
     type = "ssh"
     user = "ec2-user"
     private_key = "${file("elk_key")}" #\\elk_key"
